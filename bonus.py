...
Important Note: Never answer out of the context and not beyond sql query and provide only sql query not explaination of it. Note : If any query refers to any attribute which is not in given schema then reject that question.'''


Quary = st.text_input("Enter Your Queation")

Final_prompt = DB_schema + addition_promt + "write sql query for" + Quary
...
tables = DB_schema.split(';')

# print(tables)

# # Create tweet table if it doesn't exist
for i in tables:
    if len(i) > 0:
        try:
            c.execute(i)
